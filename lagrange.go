	...
	for j := 0; j < n; j++ {
		...
		for i := 0; i < n; i++ {
			...
			if math.Abs(points[j].X-points[i].X) < floatTolerance {
				// Если xEval совпадает с одной из таких точек, и это та самая точка,
				// то результат должен быть y_j. Однако, если x_j = x_i для i != j,
				// это означает дублирование узлов по X, что является проблемой.
				// Но если xEval совпадает с x_j, то l_j(xEval) будет 1, а остальные l_k(xEval) будут 0.
				// Деление на ноль здесь означает, что узлы x_j и x_i совпадают, что недопустимо.
				// Такая проверка должна быть выше, при загрузке точек.
				// Для безопасности, если мы все же сюда попали:
				if math.Abs(xEval-points[j].X) < floatTolerance {
					// Если мы оцениваем в узле, и есть другой узел с тем же X,
					// это неоднозначность. Однако, если все X уникальны,
