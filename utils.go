...
func ArePointsEquallySpaced(points Points) (bool, float64) {
	...
	if h <= floatTolerance {
		// Если первые две точки очень близки или совпадают по X
		// Это может быть проблемой для некоторых методов, но для равноотстояния
		// мы просто констатируем, что шаг почти нулевой.
